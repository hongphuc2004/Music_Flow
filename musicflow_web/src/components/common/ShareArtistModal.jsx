...
import {
  createArtistShareUrl,
  createArtistShareText,
  getSocialShareUrl,
  triggerNativeShare,
  getQrCodeImageUrl,
...
export default function ShareArtistModal({ open, onClose, artist }) {
  ...
  const qrShareUrl = useMemo(() => {
    if (!artist) return '';
    return createArtistShareUrl(artist, { source: 'qrcode', medium: 'offline' });
  }, [artist]);
  ...
  const shareText = useMemo(() => {
    return createArtistShareText(artist);
  }, [artist]);
  ...
  const handleCopyLink = async () => {
    try {
      ...
      setCopied(true);
      showToast({ message: 'Đã sao chép liên kết nghệ sĩ!', severity: 'success' });
  ...
  const handleSocialClick = (platformId) => {
    const customUrl = createArtistShareUrl(artist, { source: platformId, medium: 'social' });
  ...
  const handleNativeShare = async () => {
    const customUrl = createArtistShareUrl(artist, { source: 'other', medium: 'share' });
    const success = await triggerNativeShare({
      title: artist?.name || 'MusicFlow Nghệ Sĩ',
      text: shareText,
      url: customUrl,
    });
    if (!success) {
        ...
        >
          ...
          <Box sx={{ minWidth: 0, flexGrow: 1 }}>
            <Stack direction="row" spacing={0.75} alignItems="center">
              <Typography variant="subtitle2" noWrap sx={{ fontWeight: 800, fontSize: '0.98rem' }}>
                {artist.name}
              </Typography>
              {artist.isVerified && (
                <VerifiedIcon sx={{ color: '#00bcd4', fontSize: 16 }} />
              )}
            </Stack>
            <Typography variant="caption" noWrap color="text.secondary" sx={{ display: 'block', mt: 0.25 }}>
              Nghệ sĩ trên MusicFlow
            </Typography>
          </Box>
        </Stack>
        ...
        {currentTab === 0 ? (
          <Stack spacing={2.5}>
            ...
            <Box>
              <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700, mb: 0.75, display: 'block' }}>
                Liên kết chia sẻ trang nghệ sĩ:
              </Typography>
              <TextField
                fullWidth
                size="small"
          ...
          <Stack alignItems="center" spacing={2} sx={{ py: 1 }}>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ textAlign: 'center', maxWidth: 260 }}>
              Quét mã QR bằng Camera điện thoại hoặc Zalo để truy cập trang nghệ sĩ {artist.name} ngay lập tức!
            </Typography>
