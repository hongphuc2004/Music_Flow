...
class AssistantService {
  ...
  async processMessage({
    ...
    // 3. AI Orchestration using Gemini Function Calling
        ...
        try {
          ...
          if (functionCalls && functionCalls.length > 0) {
            ...
            if (name === "play_song") {
              ...
              const songsList = await Song.find({ artists: actorId }).lean();
              if (songsList.length > 0) {
                const songTitles = songsList.slice(0, 10).map((s, idx) => `${idx + 1}. ${s.title} (${s.playCount || 0} lượt nghe)`).join("\n");
                assistantText = `Bạn đã tải lên ${songsList.length} bài hát. Dưới đây là các bài hát nổi bật của bạn:\n${songTitles}`;
                clientActions.push({
                ...
                assistantText = "Bạn chưa tải lên bài hát nào trong tài khoản ca sĩ.";
              }
            } else if (name === "get_system_stats" && actorRole === "admin") {
              const [totalUsers, totalArtists, totalSongs] = await Promise.all([
                User.countDocuments({}),
                Artist.countDocuments({}),
                Song.countDocuments({ isPublic: true }),
              ]);
              assistantText = `Thống kê hệ thống MusicFlow:\n- Tổng số người dùng: ${totalUsers} người dùng\n- Tổng số nghệ sĩ: ${totalArtists} nghệ sĩ\n- Tổng số bài hát: ${totalSongs} bài hát.`;
              clientActions.push({
                type: "SHOW_SYSTEM_STATS",
                payload: { totalUsers, totalArtists, totalSongs },
              });
              metadata = { type: "get_system_stats" };
            } else if (name === "search_accounts" && actorRole === "admin") {
              const filter = {};
              if (args.email) filter.email = new RegExp(escapeRegex(args.email), "i");
              if (args.role) filter.role = args.role;

              const users = await User.find(filter).limit(10).select("name email role").lean();
              if (users.length > 0) {
                const resultText = users.map((u, idx) => `${idx + 1}. ${u.name} (${u.email}) - Vai trò: ${u.role}`).join("\n");
                assistantText = `Tìm thấy ${users.length} tài khoản phù hợp:\n${resultText}`;
                clientActions.push({
                  type: "SHOW_ACCOUNTS_SEARCH",
                  payload: { users },
                });
              } else {
                assistantText = "Không tìm thấy tài khoản nào khớp với yêu cầu tìm kiếm.";
              }
          ...
          console.warn(`Assistant Model Execution Error on ${modelName}:`, err.message);
          lastError = err;
        }
      }

      if (!success && lastError) {
        console.error("All candidates failed in Assistant processing:", lastError);
      }
    }
   ...
   */
  async generatePlaylistInternal({ prompt, mood, artistHint, userId, conversationId }) {
