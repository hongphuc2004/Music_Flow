...
def apply_audio_filters(
    ...
    """
    Applies DC offset removal, zero-phase Butterworth High-Pass filter, and peak normalization.
    Ensures 0 millisecond phase delay across all frequencies.
    """
    ...
    # 2. Butterworth High-Pass Filter (4th order, Zero-Phase Forward-Backward Filtering)
...
def convert_to_16k_mono(
    ...
    Applies DC offset removal, zero-phase high pass filtering, and waveform peak normalization.
    """
    ...
    # 1. Try ffmpeg subprocess to decode raw 16kHz mono audio
    try:
    ...
    if data is None:
        try:
            raw_data, orig_sr = sf.read(input_audio_path, dtype="float32")
            if raw_data.ndim > 1:
                raw_data = np.mean(raw_data, axis=1)
            if orig_sr != 16000:
                gcd = np.gcd(16000, orig_sr)
                data = signal.resample_poly(raw_data, 16000 // gcd, orig_sr // gcd).astype(np.float32)
            else:
                data = raw_data
        except Exception as e:
            raise RuntimeError(f"Không thể chuyển đổi định dạng âm thanh 16kHz mono: {str(e)}")

    # 3. Apply zero-phase audio filtering and normalization
