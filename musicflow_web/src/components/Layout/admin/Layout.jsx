...
function Layout({ children, title }) {
  const [desktopOpen, setDesktopOpen] = useState(
    () => localStorage.getItem('musicflow-admin-sidebar-open') !== 'false'
  );

  const handleToggleDesktopSidebar = () => {
    setDesktopOpen((prev) => {
      const next = !prev;
      localStorage.setItem('musicflow-admin-sidebar-open', String(next));
      return next;
    });
  };
  ...
  return (
    ...
    >
      <Sidebar desktopOpen={desktopOpen} onToggleDesktop={handleToggleDesktopSidebar} />
      <Header title={title} desktopSidebarOpen={desktopOpen} />
      <Box
        component="main"
        sx={{
          flexGrow: 1,
